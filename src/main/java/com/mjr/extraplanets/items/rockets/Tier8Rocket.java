...
public class Tier8Rocket extends Item implements IHoldableItem {
            ...
            if (padFound) {
                if (tile instanceof TileEntityTier2LandingPad) {
                    if (((TileEntityTier2LandingPad) tile).getDockedEntity() != null) {
                        ...
                        EntityTier8Rocket spaceship = new EntityTier8Rocket(world, centerX, centerY, centerZ, EnumRocketType.values()[stack.getItemDamage()]);
    ...
    public void addInformation(ItemStack itemStack, @Nullable World world, List<String> par2List, ITooltipFlag flagIn) {
        ...
        if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("RocketFuel")) {
            EntityTier8Rocket rocket = new EntityTier8Rocket(FMLClientHandler.instance().getWorldClient(), 0.0D, 0.0D, 0.0D, EnumRocketType.values()[itemStack.getItemDamage()]);
