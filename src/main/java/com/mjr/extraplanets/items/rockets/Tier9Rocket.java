...
public class Tier9Rocket extends Item implements IHoldableItem {
            ...
            if (padFound) {
                if (tile instanceof TileEntityTier3LandingPad) {
                    if (((TileEntityTier3LandingPad) tile).getDockedEntity() != null) {
                        ...
                        EntityTier9Rocket spaceship = new EntityTier9Rocket(world, centerX, centerY, centerZ, EnumRocketType.values()[stack.getItemDamage()]);
    ...
    public void addInformation(ItemStack itemStack, @Nullable World world, List<String> par2List, ITooltipFlag flagIn) {
        ...
        if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("RocketFuel")) {
            EntityTier9Rocket rocket = new EntityTier9Rocket(FMLClientHandler.instance().getWorldClient(), 0.0D, 0.0D, 0.0D, EnumRocketType.values()[itemStack.getItemDamage()]);
