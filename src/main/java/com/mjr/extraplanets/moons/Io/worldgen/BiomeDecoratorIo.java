...
public class BiomeDecoratorIo extends BiomeDecoratorSpace {
    // Custom code
    private static final List<CustomOreGenerationData> customOreList = CustomOreGeneration.getOresForObject("Io");
        ...
        if (!this.isDecorating) {
            ...
            if (customOreList != null) {
                for (CustomOreGenerationData ore : customOreList) {
                    ...
                    int minY = ore.minY;
                    int maxY = ore.maxY;
                    WorldGenerator blockGeneration = new WorldGenMinableMeta(ore.block, ore.clusterSize, ore.meta, ore.useMeta, ExtraPlanets_Blocks.IO_BLOCKS, 2);
            ...
            if (((BiomeAdaptive) this.getCurrentWorld().getBiome(new BlockPos(this.posX, 0, this.posZ))).isInstance(BiomeGenIOBurningPlains.class)) {
                for (int i = 0; i < this.LakesPerChunk * 2; ++i) {
