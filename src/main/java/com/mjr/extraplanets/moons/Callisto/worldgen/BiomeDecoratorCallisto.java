...
public class BiomeDecoratorCallisto extends BiomeDecoratorSpace {
    // Custom code
    private static final List<CustomOreGenerationData> customOreList = CustomOreGeneration.getOresForObject("Callisto");
    ...
    protected void decorate() {
        ...
        if (customOreList != null) {
            for (CustomOreGenerationData ore : customOreList) {
                ...
                int minY = ore.minY;
                int maxY = ore.maxY;
                WorldGenerator blockGeneration = new WorldGenMinableMeta(ore.block, ore.clusterSize, ore.meta, ore.useMeta, ExtraPlanets_Blocks.CALLISTO_BLOCKS, 2);
