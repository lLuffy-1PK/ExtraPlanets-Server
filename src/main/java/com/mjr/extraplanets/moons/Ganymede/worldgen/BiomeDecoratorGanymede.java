...
public class BiomeDecoratorGanymede extends BiomeDecoratorSpace {
    // Custom code
    private static final List<CustomOreGenerationData> customOreList = CustomOreGeneration.getOresForObject("Ganymede");
        ...
        if (!this.isDecorating) {
            ...
            if (customOreList != null) {
                for (CustomOreGenerationData ore : customOreList) {
                    ...
                    int minY = ore.minY;
                    int maxY = ore.maxY;
                    WorldGenerator blockGeneration = new WorldGenMinableMeta(ore.block, ore.clusterSize, ore.meta, ore.useMeta, ExtraPlanets_Blocks.GANYMEDE_BLOCKS, 2);
            ...
            if (Config.GENERATE_GANYMEDE_ICE_CHUNKS) {
                for (i = 0; i < 3; ++i) {
                    if (this.rand.nextInt(5) == 1) {
                        WorldGenUtilities.generateStructureWithRangeY(new WorldGenSphere(false, "extraplanets", Blocks.ICE.getDefaultState(), 12, 2), this.currentWorld, this.rand, new BlockPos(this.posX, 0, this.posZ), 6, 60);
                    }
                }
            }
